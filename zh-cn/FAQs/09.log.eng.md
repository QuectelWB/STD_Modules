...
Some unknown reason causes the module to malfunction. We need catch logs. Include


	Modem log （Alsoe called QLog、QXDM log）
	Dump log
	Demsg log （Host AP Side，Host Driver log）
	tcpdump
	usb log （usbmon log）
	Android RIL log
	Modules AP side AP kernel log（debug port log）
